...
VkDescriptorPool Minty::Shader::create_pool(uint32_t const set)
{
	...
	VK_ASSERT(vkCreateDescriptorPool(RenderEngine::instance().get_device(), &poolInfo, nullptr, &descriptorPool), "Failed to create descriptor pool.");
...
std::array<VkDescriptorSet, MAX_FRAMES_IN_FLIGHT> Minty::Shader::create_descriptor_sets(VkDescriptorPool const& pool, uint32_t const set)
{
	...
	VK_ASSERT(vkAllocateDescriptorSets(RenderEngine::instance().get_device(), &allocInfo, descriptorSets.data()), std::format("Failed to allocate descriptor sets for descriptor set layout {}.", set));
