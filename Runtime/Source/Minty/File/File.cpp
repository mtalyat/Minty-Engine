...
std::vector<String> Minty::File::read_lines(Size const count)
{
    std::vector<String> result;
    if (count > 0) result.reserve(count);

    String line;
    for (Size i = 0; i < count && !end_of_file() && read_line(line); i++)
    {
...
std::vector<String> Minty::File::read_lines(Path const& path, Size const count)
{
