...
namespace Minty
{
	...
	private:
		static std::map<String const, SystemFunc const> m_systemTypes;

		// the systems to manage
		std::map<int, std::set<System*>> m_orderedSystems;
		std::map<String const, System*> m_allSystems;
		std::unordered_map<TypeID, System*> m_typeLookup;
	...
	{
		auto found = m_typeLookup.find(typeid(T));

		if (found != m_typeLookup.end())
		{
			return static_cast<T*>(found->second);
		}
	...
	{
		if (m_systemTypes.contains(name))
		{
			MINTY_INFO(std::format("System {} already registered.", name));
			return;
		}

		m_systemTypes.emplace(name, [](Scene& scene) { return new T(scene); });
