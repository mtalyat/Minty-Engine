...
std::string const minty::Vector2::to_string() const
{
    return std::format("Vector2({0}, {1})", x, y);
}

minty::Vector2 minty::Vector2::to_cartesian(minty::Vector2 const& polar)
{
    return minty::Vector2(math::cos(polar.x) * polar.y, math::sin(polar.x) * polar.y);
}

minty::Vector2 minty::Vector2::to_polar(minty::Vector2 const& cartesian)
{
